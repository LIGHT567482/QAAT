...
> ## What is in v1, and what is deferred to v2
>
> **v1 runs as an INTERNAL Quality Assurance system.** Live and supported: lecturer attendance
> (coordinator gate + QA monitor round), employee attendance, QA monitor coverage and briefings,
> presence disputes, the timetable, org/oversight dashboards, and the reports drawn from them.
>
> **Deferred to v2 — the routes below exist in this catalog but are commented out in
> `backend/api-gateway/internal/router/router.go` and will answer 404:**
>
> - **The whole student module** — every `/api/v1/student/*` path, student check-in
>   (`/checkin`, `/attendance/geo-checkin`, `/attendance/offline-batch`), `/attendance/attempts`,
>   `/eligibility/*`, `/admin/students*`, `/coordinator/students`, `/coordinator/attendance`,
>   `/lecturer/attendance`, `/lecturer/roster`, `/lecturer/sessions/{id}/students`,
>   `/org/at-risk`, `/sessions/{id}/checkin-code`, `/sessions/{id}/roster`, and every
>   `/dashboard/qa/student-attendance*`, `/dashboard/dqa/{unit-attendance,course-health,trends,
>   ineligible,eligibility-all}`, `/dashboard/qa/device-reset`,
>   `/dashboard/qa/attendance-correction` and `/reports/dqa/eligibility.csv`.
> - **The U-Panel integration** — `/dashboard/upanel/attendance` and the outbound client behind
>   it. See `ATTENDANCE_API.md`.
>
> Nothing is deleted: handlers still compile and are still tested, so restoring a module is
> uncommenting its route rather than rewriting it.
...
> **⛔ DEFERRED TO V2.** Every route in this section is commented out in the gateway. A student
> account still authenticates, but the phone app answers with a "not in this version" screen and
> the web portal with the same. See the v1 scope note at the top of this file.
