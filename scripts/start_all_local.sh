...
(cd "$REPO/backend/auth-service" && go build -o auth-service . 2>&1)
...
PORT="8090" \
"$REPO/backend/auth-service/auth-service" &>/tmp/auth-service.log &
PIDS+=($!)
...
(cd "$REPO/backend/api-gateway" && go build -o api-gateway . 2>&1)
DB_URL="postgres://qaat:changeme_db@127.0.0.1:5434/qaat?sslmode=disable" \
REDIS_URL="redis://:changeme_redis@127.0.0.1:6379/0" \
RSA_PUBLIC_KEY_PATH="$REPO/keys/auth_public.pem" \
...
PORT="8080" \
"$REPO/backend/api-gateway/api-gateway" &>/tmp/api-gateway.log &
PIDS+=($!)
info "api-gateway → :8080 (PID ${PIDS[-1]})"

# ── sync-receiver ─────────────────────────────────────────────────────────────
info "Building sync-receiver..."
(cd "$REPO/backend/sync-receiver" && go build -o sync-receiver . 2>&1)
...
PORT="8083" \
"$REPO/backend/sync-receiver/sync-receiver" &>/tmp/sync-receiver.log &
PIDS+=($!)
