...
func TestVerifyRealSignature(t *testing.T) {
	...
	if q.StudentID != "CS-2024-0001" || q.TenantID != "11111111-1111-1111-1111-111111111111" || q.FullName != "Jane Doe" {
...
// Anti-proxy regression: tampering with full_name must be detected.
// A fraudster cannot substitute their name into someone else's QR.
func TestTamperedFullNameRejected(t *testing.T) {
	raw, pub := loadFixture(t)
	tampered := strings.Replace(raw, "Jane Doe", "Evil Proxy", 1)
	if tampered == raw {
		t.Fatal("test setup: nothing replaced")
	}
	q, err := ParseQR(tampered)
	...
	if err := q.VerifySignature(pub); err == nil {
		t.Fatal("tampered full_name passed signature verification — proxy fraud undetected")
	}
}
...
// TestParseQRToken proves the URL captive-portal round-trip: JSON → base64url → ParseQRToken → same struct.
func TestParseQRToken(t *testing.T) {
	raw, _ := loadFixture(t)
	token := base64.RawURLEncoding.EncodeToString([]byte(raw))
	q, err := ParseQRToken(token)
	if err != nil {
		t.Fatalf("ParseQRToken: %v", err)
	}
	if q.StudentID != "CS-2024-0001" {
		t.Fatalf("student_id round-trip failed: got %q", q.StudentID)
	}
	if q.CourseID != "CS-COURSE-001" {
		t.Fatalf("course_id round-trip failed: got %q", q.CourseID)
	}
	if q.FullName != "Jane Doe" {
		t.Fatalf("full_name round-trip failed: got %q", q.FullName)
	}
}
