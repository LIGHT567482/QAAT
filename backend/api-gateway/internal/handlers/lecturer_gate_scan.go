...
func LecturerGateScan(adminPool *pgxpool.Pool, rdb *redis.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// (2b) The mandatory same-LAN gate is REMOVED along with the hotspot it depended on.
		//
		// It was the strongest check here: a lecturer had to be on the radio in the room, so a
		// relayed staff ID and a photographed code were not enough. What is left is the live
		// 10-second digit code above, which still requires reading a screen that is physically in
		// the room — weaker, because a code can be read out over a phone call, and the whole
		// reason the LAN check existed. Recorded here so the gap is deliberate and visible rather
		// than discovered later by someone wondering why the check-in is easier than it was.
		_ = coordinatorIP
