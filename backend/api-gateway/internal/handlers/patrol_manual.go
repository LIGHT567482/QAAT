...
func PatrolManualEntry(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// The same in-app alert the round writes for a timetabled slot, so an off-timetable
		// manual verdict is heard exactly as loud: the lecturer's inbox, with the appeal
		// pointed at this exact lecture when the record is NOT TAUGHT (patrol_absent_alert.go).
		// Best-effort throughout: the record is already committed, and a failed courtesy must
		// not fail this response.
		var lecUser string
		_ = conn.QueryRow(r.Context(), `
			SELECT COALESCE(l.user_id::text,'')
			  FROM lecturers l
			 WHERE l.tenant_id = $1 AND btrim(lower(l.staff_id)) = btrim(lower($2)) AND $2 <> ''`,
			tenantID, lecturerKey).Scan(&lecUser)
