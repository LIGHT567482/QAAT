	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := adminPool.Query(r.Context(), `
			SELECT se.student_id, se.full_name, se.email,
			       se.course_id, COALESCE(c.name, '') AS course_name,
			       -- COALESCED because a student can legitimately have none of these yet. A record
			       -- created from an imported attendance feed carries a name and a registration
			       -- number and nothing else, and a NULL here used to fail the row scan — which
			       -- aborted the loop, so ONE unassigned student truncated the entire roll to the
			       -- rows before it. The list silently showed a fraction of the students.
			       COALESCE(se.current_year, 0), COALESCE(se.semester, 0),
			       COALESCE(se.academic_year, ''),
			       COALESCE(se.intake_session::text, ''), COALESCE(se.enrollment_status::text, ''),
			       COALESCE(se.level,'')
			FROM students_extended se
			-- LEFT JOIN: an unassigned student has a placeholder course, but a student whose course
			-- was deleted has none at all, and dropping them from the roll would hide exactly the
