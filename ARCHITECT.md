...
[Session Active — Fully Offline]
BLE Beacon ──(RSSI)──► PWA BLE Engine ──► Session State Machine
Lecturer QR ─────────► PWA QR Validator ──► Gate-Open event
Student QR ──────────► PWA QR Validator
                         ├─ RSA verify
                         ├─ Roster lookup
                         ├─ BLE proximity check
                         ├─ Fingerprint binding check
                         └─ Duplicate check
                              └──► Attendance_Log (IndexedDB)
