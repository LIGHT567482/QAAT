...
export type SessionEvent =
  | { type: 'SELECT_UNIT'; unit_id: string; unit_name: string; venue_id: string; beacon_uuid: string }
  | { type: 'BLE_DETECTED' }
...
export const sessionMachine = createMachine(
  {
    ...
    states: {
      ...
      PENDING_LECTURER: {
        ...
        on: {
          BLE_DETECTED: { actions: assign({ beacon_detected: true }) },
          LECTURER_GATE_OPEN: {
            target: 'ACTIVE',
  ...
  {
    guards: {
      bleDetected: ({ context }) => context.beacon_detected,
    },
  },
)
