...
<html lang="en"><head>
  ...
  <script>
    ...
    // Best-effort proximity: measure beacon RSSI on this phone if Web Bluetooth
    // scanning is available; otherwise null and server policy decides (F-06).
    async function measureRSSI(){
      try {
        if (!navigator.bluetooth || !navigator.bluetooth.requestLEScan) return null;
        const readings = [];
        navigator.bluetooth.addEventListener('advertisementreceived', ev => readings.push(ev.rssi));
        await navigator.bluetooth.requestLEScan({ acceptAllAdvertisements: true, keepRepeatedDevices: true });
        await new Promise(r => setTimeout(r, 3000));
        return readings.length ? Math.round(readings.reduce((a,b)=>a+b,0)/readings.length) : null;
      } catch { return null; }
    }
    document.getElementById('qr').addEventListener('change', async e => {
      ...
      const rssi = await measureRSSI();
      if (rssi !== null) fd.append('rssi', String(rssi));
...
// The SW can't run the 8-step validator (no DOM / BLE / session context). It
...
function requestValidation(
  ...
  return new Promise((resolve) => {
    ...
    validationChannel.postMessage({
      type: 'QAAT_LAN_VALIDATE', requestId, sessionId, image, fingerprintHash, rssi,
    })
  })
}
...
async function handleStudentSubmit(req: Request): Promise<Response> {
  ...
  // Fingerprint + RSSI are measured on the student's phone and travel with the
  // submission so the validator binds THIS handset, not the Coordinator's.
  const fingerprintHash = String(form.get('fingerprint') ?? '')
  const rssiRaw = form.get('rssi')
  const rssi = rssiRaw === null ? null : Number(rssiRaw)
  ...
  const result = await requestValidation(
    sessionId, image, fingerprintHash, Number.isFinite(rssi as number) ? rssi : null,
  )
