    ...
    <div style={{ maxWidth: 520 }}>
      ...
      <div style={{ display: 'grid', gap: 20 }}>
        <Field
          ...
          hint="Minimum % required for exam eligibility"
          value={form.attendance_threshold}
          min={1} max={100}
          onChange={v => setForm(f => f && ({ ...f, attendance_threshold: v }))}
        />
        <Field
        ...
        />
        <Field
          label="BLE RSSI threshold (dBm)"
          hint="Minimum signal strength (e.g. -65 = ~3 m range)"
          value={form.rssi_threshold_dbm}
          min={-100} max={-30}
          onChange={v => setForm(f => f && ({ ...f, rssi_threshold_dbm: v }))}
        />
      </div>
