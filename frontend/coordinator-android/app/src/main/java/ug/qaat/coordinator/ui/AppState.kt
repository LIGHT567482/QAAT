...
object AppState {
    ...
    // The register currently open on this phone. Replaces the hotspot-era notion of "the session
    // this device is hosting": nothing is hosted any more, so what matters is which server-side
    // session this lecturer or coordinator opened and is reading a code out for.
    var activeSessionId by mutableStateOf<String?>(null)
    var activeSessionCode by mutableStateOf<String?>(null)
    ...
    // (Retired with the hotspot: this mirrored how full the class Wi-Fi was, so the coordinator
    ...
    // for half a minute. Retired with the hotspot; kept only so existing screens still compile
