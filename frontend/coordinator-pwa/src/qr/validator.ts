...
export type RejectionReason =
  ...
  | 'PROXIMITY_FAILED'
  | 'DEVICE_MISMATCH'
  | 'DEVICE_BELONGS_TO_ANOTHER_STUDENT'
  | 'DUPLICATE_SCAN'
  | 'DEVICE_ALREADY_USED'
  | 'SESSION_NOT_ACTIVE'
  | 'GATE_NOT_OPEN'
...
// DeviceContext carries the device-identity signal measured on the *student's*
// handset (when checking in via the LAN page) and forwarded with the scan. For
// the Coordinator camera fallback it carries the Coordinator device's own value.
// Binding the fingerprint to the submitting device — rather than recomputing it
// on whatever device runs the validator — is what makes the fingerprint check
// meaningful (F-01). Proximity is proven separately by being on the coordinator's
// LAN (the hotspot) plus the live room code.
...
export async function validateStudentQR(
  ...
  // Pre-compute the student hash once — used by Steps 6a, 4, 4b, 6, 7, 8.
  const studentIdHash = await hmacHex(session.studentHashKey, payload.student_id)

  // ── Step 6a: cross-student device lock ────────────────────────────────────
  // Check whether this device is already bound to a DIFFERENT student. If so,
  // reject immediately — a classmate cannot check in using someone else's QR
  // URL on their own phone (proxy attendance detection).
  const deviceBinding = await db.hardware_vault
    .where('fingerprint_hash').equals(device.fingerprintHash).first()
  if (deviceBinding && deviceBinding.student_id_hash !== studentIdHash) {
    return { status: 'REJECTED', reason: 'DEVICE_BELONGS_TO_ANOTHER_STUDENT' }
  }
  ...
  // ── Step 5: Hardware fingerprint check ────────────────────────────────────
  ...
  // ── Step 7: One-device-per-session check ──────────────────────────────────
  ...
  return { status: 'PRESENT', studentIdHash }
}
